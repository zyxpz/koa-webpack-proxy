...
const { isRemote, isMatch, getParams, getRes } = require('./utils');
	...
	return {
		...
		dealLocalResponse(_req, reqBody, callback) {
			...
			return batchMatch(req, getProxyConfig(), (val, pattern) => {
				...
				if (typeof val === 'function') {
					console.info(`${req.method} ${req.url} matches ${pattern}, respond with custom function`);
				...
				if (typeof val === 'string' && !isRemote(val)) {
					console.info(`${req.method} ${req.url} matches ${pattern}, respond with local file`);
				...
				if (isPlainObject(val) || Array.isArray(val)) {
					console.info(`${req.method} ${req.url} matches ${pattern}, respond with object or array`);
    ...
    */
		...
		replaceRequestOption(req, option) {
			...
			batchMatch(req, proxyConfig, (val, pattern) => {
				if (isRemote(val)) {
					console.info(`${req.method} ${req.url} matches ${pattern}, forward to ${val}`);
					isModified = true;
					setOption(val);
					return true;
				}
